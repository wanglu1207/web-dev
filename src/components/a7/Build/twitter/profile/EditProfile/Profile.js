...
const EditProfile = () => {
    let [name, setName] = useState({name:profileData.name})
    let [bio, setBio] = useState({bio: profileData.bio})
    let [local, setLocal] = useState({local: profileData.location})
    let [web, setWeb] = useState({web : profileData.website})
    let [birth, setBirth] = useState({birthday: profileData.dateOfBirth});
